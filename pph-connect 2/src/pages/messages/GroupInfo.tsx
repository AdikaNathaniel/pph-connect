    ...
    try {
      const { data, error } = await supabase
        ...
        .select(`
          ...
          role,
          joined_at,
          worker:workers!group_members_user_id_fkey(full_name, email_personal, email_pph, worker_role)
        `)
        .eq('group_id', groupId)
        .is('left_at', null)
      ...
      const formattedMembers: GroupMember[] = (data || []).map((m: any) => ({
        ...
        full_name: m.worker?.full_name || 'Unknown',
        email: m.worker?.email_pph || m.worker?.email_personal || '',
        user_role: m.worker?.worker_role || 'unknown',
      }));
