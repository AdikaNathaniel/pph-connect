  ...
  const fetchMessages = async (userId: string, loadMore = false) => {
    ...
    const { data, error } = await supabase
      ...
      .select(`
        ...
        sent_at,
        attachments,
        sender:workers!messages_sender_id_fkey(full_name, worker_role),
        message_recipients(
          recipient_id,
          read_at,
          recipient:workers!message_recipients_recipient_id_fkey(full_name)
        )
      `)
    ...
    const formattedMessages: MessageDetails[] = (data || []).map((msg: any) => ({
      ...
      sender_role: msg.sender?.worker_role || 'unknown',
      content: msg.content,
      sent_at: msg.sent_at,
      attachments: msg.attachments || [],
      read_receipts: msg.message_recipients.map((mr: any) => ({
        recipient_id: mr.recipient_id,
        recipient_name: mr.recipient?.full_name || 'Unknown',
        read_at: mr.read_at
      }))
    }));
