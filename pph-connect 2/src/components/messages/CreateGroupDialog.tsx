...
const CreateGroupDialog: React.FC<CreateGroupDialogProps> = ({
  ...
  const fetchAvailableUsers = async () => {
    ...
    try {
      ...
      // Fetch all workers except the current user
      const { data, error } = await supabase
        .from('workers')
        .select('id, full_name, email_pph, email_personal, worker_role, status')
      ...
      const mappedUsers: Profile[] = (data || []).map((worker) => ({
        id: worker.id,
  ...
  useEffect(() => {
    if (!userSearchQuery.trim()) {
      ...
      const filtered = availableUsers.filter((worker) =>
        worker.full_name.toLowerCase().includes(userSearchQuery.toLowerCase()) ||
        worker.email.toLowerCase().includes(userSearchQuery.toLowerCase()) ||
        worker.worker_role.toLowerCase().includes(userSearchQuery.toLowerCase())
      );
