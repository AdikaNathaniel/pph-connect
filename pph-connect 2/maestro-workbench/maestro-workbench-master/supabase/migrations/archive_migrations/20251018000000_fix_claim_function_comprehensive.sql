...
-- First, clean up any stuck reservations for the test worker
-- This is safe because we're only clearing 'assigned' status tasks that should have been released
UPDATE public.tasks
SET status = 'pending',
    assigned_to = NULL,
    assigned_at = NULL
WHERE status IN ('assigned', 'in_progress')
  ...
  AND (
    assigned_at IS NULL 
    OR assigned_at < NOW() - INTERVAL '2 hours'
  );
...
BEGIN
  ...
  -- First, clean up any expired reservations for this worker
  ...
  -- Attempt to resume existing valid reservation for this worker
  SELECT t.*
  INTO resumed_task
  ...
  -- Clean up expired reservations for this project
  UPDATE public.tasks AS t
  SET status = 'pending',
      assigned_to = NULL,
      assigned_at = NULL
  ...
  -- Try to insert the task, handling the unique constraint gracefully
  ...
  EXCEPTION 
    WHEN unique_violation THEN
      -- If we hit the unique constraint, it means this worker already has an active reservation
      ...
      IF FOUND THEN
        ...
        RETURN;
      ELSE
        -- This shouldn't happen, but if it does, return empty
        RETURN;
      END IF;
  END;

  -- Return the question data for the newly created reservation
  IF inserted_task_id IS NOT NULL THEN
    RETURN QUERY SELECT
      selected_question.id,
      selected_question.project_id,
