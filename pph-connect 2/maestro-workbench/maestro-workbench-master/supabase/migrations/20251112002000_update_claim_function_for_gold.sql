-- Migration: Inject gold standards during task reservation
-- Created: 2025-11-12
-- Purpose: Randomly surface gold standard questions when workers claim tasks
--
-- ============================================================================
...
DECLARE
    ...
    v_gold_available BOOLEAN := FALSE;
    v_select_gold BOOLEAN := FALSE;
BEGIN
    ...
    -- Determine if gold standard questions are available
    SELECT EXISTS (
        SELECT 1
        FROM public.questions q
        WHERE q.project_id = p_project_id
          AND q.is_gold_standard = TRUE
    ...
    INTO v_gold_available;

    IF v_gold_available THEN
        v_select_gold := random() <= 0.15;
    END IF;

    -- Attempt to select a gold standard question at random
    IF v_select_gold THEN
        SELECT q.*
    ...
    -- Fall back to standard queue ordering when gold selection skipped or unavailable
