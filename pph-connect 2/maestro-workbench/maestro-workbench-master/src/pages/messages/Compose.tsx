...
const Compose: React.FC = () => {
  ...
  const fetchCurrentUserAndRecipients = async () => {
    try {
      ...
      // Fetch available recipients (all users except self and suspended)
      let query = supabase
        .from('profiles')
  ...
  useEffect(() => {
    if (!recipientSearchQuery.trim()) {
      ...
      const filtered = availableRecipients.filter((user) =>
        user.full_name.toLowerCase().includes(recipientSearchQuery.toLowerCase()) ||
        user.email.toLowerCase().includes(recipientSearchQuery.toLowerCase()) ||
        user.role.toLowerCase().includes(recipientSearchQuery.toLowerCase())
      );
                ...
                <>
                  ...
                  {selectedRecipientsList.length > 0 && (
                    <div className="flex flex-wrap gap-2 mt-2">
                      ...
                      {selectedRecipientsList.map(user => (
